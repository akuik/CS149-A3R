...
void saxpyCuda(int N, float alpha, float* xarray, float* yarray, float* resultarray) {
    ...
    // Calculate and print the effective bandwidth of the kernel execution
    double kernelBandwidth = GBPerSec(sizeof(float) * 3 * N, kernelDuration);
    printf("Kernel execution time: %.3f ms\t\t[%.3f GB/s]\n", 1000.f * kernelDuration, kernelBandwidth);
